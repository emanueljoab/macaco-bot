                ...
                async (err, rows) => {
                    ...
                    const rankings = await Promise.all(
                        rows.map(async (row, index) => {
                            ...
                            const username =
                                row.username ||
                                (user ? escapeMarkdown(user.displayName) : await translate('jokenpo', 'unknown user'));
            ...
            collector.on("collect", async (interaction) => {
                ...
                if (Object.keys(choices).length === 2) {
                    ...
                    if (choices[sortedPlayer1.id] === choices[sortedPlayer2.id]) {
                        ...
                        (choices[sortedPlayer1.id] === await translate('jokenpo', 'id rock') &&
                            choices[sortedPlayer2.id] === await translate('jokenpo', 'id scissors')) ||
                        (choices[sortedPlayer1.id] === await translate('jokenpo', 'id paper') &&
                            choices[sortedPlayer2.id] === await translate('jokenpo', 'id rock')) ||
                        (choices[sortedPlayer1.id] === await translate('jokenpo', 'id scissors') &&
                            choices[sortedPlayer2.id] === await translate('jokenpo', 'id paper'))
                    ) {
