    ...
    try {
        ...
        if (args.length > 0 && args[0].toLowerCase() === "rank") {
                ...
                async (err, rows) => {
                    if (err) {
                        ...
                        return message.reply(
                            await translate("jokenpo", "reply rank error")
                        );
                    }
                    ...
                    const embed = new EmbedBuilder()
                        .setTitle(await translate("jokenpo", "jokenpo ranking"))
                        .setDescription(
                            rankings.join("\n") ||
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(
                    await translate(
                        "jokenpo",
                        "challenge",
                        escapeMarkdown(player1.username),
                        escapeMarkdown(player2.username)
                    )
                );
            ...
            const filter = (interaction) =>
                ...
                [sortedPlayer1.id, sortedPlayer2.id].includes(
                    interaction.user.id
                );
            ...
            collector.on("collect", async (interaction) => {
                ...
                if (Object.keys(choices).length < 2) {
                    ...
                    embed.setDescription(
                        await translate(
                            ...
                            escapeMarkdown(jogadorEsperando.username)
                        )
                    );
                ...
                if (Object.keys(choices).length === 2) {
                    ...
                    if (
                        choices[sortedPlayer1.id] === choices[sortedPlayer2.id]
                    ) {
                        resultado = await translate("jokenpo", "tie");
                        console.log(
                            `${new Date().toLocaleString(
                                "pt-BR"
                            )} | Jokenpo: Empate entre ${
                                sortedPlayer1.username
                            } e ${sortedPlayer2.username}`
                        );
                    } else if (
                        (choices[sortedPlayer1.id] ===
                            (await translate("jokenpo", "id rock")) &&
                            choices[sortedPlayer2.id] ===
                                (await translate("jokenpo", "id scissors"))) ||
                        (choices[sortedPlayer1.id] ===
                            (await translate("jokenpo", "id paper")) &&
                            choices[sortedPlayer2.id] ===
                                (await translate("jokenpo", "id rock"))) ||
                        (choices[sortedPlayer1.id] ===
                            (await translate("jokenpo", "id scissors")) &&
                        ...
                        resultado = await translate(
                            "jokenpo",
                            "player 1 win",
                            escapeMarkdown(sortedPlayer1.username)
                        );
                        console.log(
                            `${new Date().toLocaleString("pt-BR")} | Jokenpo: ${
                                sortedPlayer1.username
                            } venceu ${sortedPlayer2.username}`
                        );
                    } else {
                        resultado = await translate(
                            "jokenpo",
                    ...
                    let historyMessage = await translate(
                        "jokenpo",
                        "no history"
                    );
                    if (
                        ...
                        console.log(
                            `Vitória registrada para ${sortedPlayer1.username}`
                        );
                    } else if (
                        resultado.includes(
                            escapeMarkdown(sortedPlayer2.username)
                        ...
                        console.log(
                            `Vitória registrada para ${sortedPlayer2.username}`
                        );
                    } else {
                        historyMessage = await atualizarHistorico(
                            sortedPlayer1.id,
